...
public class CardVisual : MonoBehaviour
{
    ...
    public void Initialize(CardHandler target, int index = 0)
    {
        //Declarations
        parentCard = target;
    ...
    private void FollowRotation()
    {
        ...
        transform.eulerAngles = new Vector3(transform.eulerAngles.x, transform.eulerAngles.y, Mathf.Clamp(rotationDelta.x, -45, 45));
    }
    ...
    private void CardTilt()
    {
        ...
        tiltParent.eulerAngles = new Vector3(tiltParent.eulerAngles.x, tiltParent.eulerAngles.y, lerpZ);
    }

    private void Select(CardHandler card, bool state)
    {
        DOTween.Kill(2, true);
        float dir = state ? 1 : 0;
    ...
    private void EndDrag(CardHandler card)
    {
        canvas.overrideSorting = false;
        transform.DOScale(1, scaleTransition).SetEase(scaleEase);
    }

    private void PointerEnter(CardHandler card)
    {
        if (scaleAnimations)
            transform.DOScale(scaleOnHover, scaleTransition).SetEase(scaleEase);
    ...
    private void PointerExit(CardHandler card)
    {
        if (!parentCard.wasDragged)
            transform.DOScale(1, scaleTransition).SetEase(scaleEase);
    }

    private void PointerUp(CardHandler card, bool longPress)
    {
        if (scaleAnimations)
    ...
    private void PointerDown(CardHandler card)
    {
        if (scaleAnimations)
            transform.DOScale(scaleOnSelect, scaleTransition).SetEase(scaleEase);
